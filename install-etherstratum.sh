...
sudo apt install -y openvpn vim emacs nano 
sudo apt install -y flatpak snap kitty git nala
...
sudo pip install juypter fastapi httpie tqdm
clear
...
NAME="EtherStratum"
PRETTY_NAME="EtherStratum 1.1.0 (Debian-based)"
VERSION="1.1.0"
VERSION_ID="1.1.0"
VERSION_CODENAME="kirbensis"
ID=etherstratum
ID_LIKE=debian
...
DISTRIB_CODENAME=kirbensis
DISTRIB_DESCRIPTION="EtherStratum 1.1.0 (Debian-based)"
EOF

sudo tee "$TARGET/etc/issue" > /dev/null <<'EOF'
EtherStratum GNU/Linux 1.1.0 (Kirbensis)
Welcome to EtherStratum — run 'etherstratum' for quick info.
EOF

sudo tee "$TARGET/etc/issue.net" > /dev/null <<'EOF'
EtherStratum GNU/Linux 1.1.0 (Kirbensis) - Authorized users only.
EOF

sudo tee "$TARGET/etc/motd" > /dev/null <<'EOF'
Welcome to EtherStratum Kirbensis (1.1.0)
